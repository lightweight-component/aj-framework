...
public class Image {
    ...
    /**
     * 图像顺时针旋转270度
     * 对图像矩阵原地旋转(In-place matrix transposition)的好处就是不用占用额外内存，
     ...
     * 从 RGBA 格式转 BufferedImage 的实现如下，注意，这个实现实际只保留了 Red,Green,Blue 三个颜色通道数据，删除了 alpha 通道。
     *
     ...
     * @see Raster#createInterleavedRaster(DataBuffer, int, int, int, int, int[], Point)
     * @see BufferedImage#BufferedImage(ColorModel, WritableRaster, boolean, java.util.Hashtable)
     */

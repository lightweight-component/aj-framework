...
public class FileUtils {
    /*
     * 不论在windows还是linux下,仅凭字符串比较判断两个文件路径是否相等是不靠谱的。
     * 因为有link，Disk map等技术的存在，两个不同的路径有可能指向同一个文件。
     ...
     * 但是对于文件夹，这个办法并不能用来测试文件夹是否可以创建子文件夹或文件。
     * 比如对于匿名(只读)访问一个网络共享文件夹,isWritable返回是true
     * 所以如果想判断一个文件夹是不是真的可写，这个办法是不靠谱的。
     * 怎么办呢？看来只有去尝试创建文件和文件夹才能真判断文件夹是否可写了，

package com.ajaxjs.pay.core.utils;
	...
	public static <V extends ResultCheck> V retryOnException(int retryLimit, java.util.concurrent.Callable<V> retryCallable) {
		...
		for (int i = 0; i < retryLimit; i++) {
			try {
				...
				log.warn("retry on {} times v = {}", i + 1, v == null ? null : v.getJson(), e);
			}

			if (null != v && v.matching())
				break;
			log.error("retry on {} times but not matching v = {}", i + 1, v == null ? null : v.getJson());
		}
		return v;
	}

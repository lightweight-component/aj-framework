...
public class XmlEncryptUtil {
	...
	public static String encrypt(String rsaPrivateKey, String strDesKey, String genSignStr) {
		...
		if (StrUtil.hasText(rsaPrivateKey) && StrUtil.hasText(strDesKey) && StrUtil.hasText(genSignStr)) {
			...
			try {
				...
				genSignStr = JdPayXmlUtil.formatXmlStr(genSignStr);
	...
	public static String decrypt(String rsaPubKey, String strDesKey, String encrypt) {
		...
		try {
			reqBody = ThreeDesUtil.decrypt4HexStr(RsaUtil.decryptBASE64(strDesKey), new String(Base64.decodeBase64(encrypt), StandardCharsets.UTF_8));
			String inputSign = JdPayXmlUtil.getXmlElm(reqBody, SIGN);
			reqBody = JdPayXmlUtil.addXmlHead(reqBody);
			reqBody = JdPayXmlUtil.formatXmlStr(reqBody);
	...
	public static String decrypt(String rsaPubKey, String reqBody) {
		...
		try {
			...
			req = JdPayXmlUtil.formatXmlStr(req);

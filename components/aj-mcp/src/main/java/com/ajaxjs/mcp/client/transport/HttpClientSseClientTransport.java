...
public class HttpClientSseClientTransport implements McpClientTransport {
    ...
    public HttpClientSseClientTransport(HttpClient.Builder clientBuilder, String baseUri, String sseEndpoint,
        ...
        Objects.requireNonNull(objectMapper, "ObjectMapper must not be null");
         Objects.requireNonNull(baseUri, "baseUri must not be empty");
         Objects.requireNonNull(sseEndpoint, "sseEndpoint must not be empty");
            ...
            public void onEvent(SseEvent event) {
                ...
                try {
                    if (ENDPOINT_EVENT_TYPE.equals(event.type())) {
                        ...
                        handler.apply(CompletableFuture.completedFuture(message)).subscribe();
                    } else {
                        log.error("Received unrecognized SSE event type: {}", event.type());
                    }
                } catch (IOException e) {
    ...
    public Mono<Void> sendMessage(JSONRPCMessage message) {
        ...
        try {
            ...
            return Mono.fromFuture(httpClient.sendAsync(request, HttpResponse.BodyHandlers.discarding()).thenAccept(response -> {

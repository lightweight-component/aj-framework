...
public class ConfigTreeEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    /**
     * Append additional {@link PropertySource} instances for config tree files that have loadable extensions
     * (e.g. {@code .yml}, {@code .properties}) so that keys can be referenced with a path-like prefix.
     *
    ...
    public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication application) {
        ...
        if (configTreeProp.isPresent()) {
            ...
            Map<String, Path> allPropertyFiles = ConfigurationUtils.findAllKeyAndFilesInConfigTree(source);
                    ...
                    for (PropertySourceLoader loader : loaders) {
                        if (Arrays.asList(loader.getFileExtensions()).contains(extension)) {
                            ...
                            if (!newPropsList.isEmpty()) {
                                PropertySource<?> ps = ConfigurationUtils.addConfigPropPrefix((OriginTrackedMapPropertySource) newPropsList.get(0), prefix);

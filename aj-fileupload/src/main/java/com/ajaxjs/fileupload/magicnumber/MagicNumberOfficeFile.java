...
public class MagicNumberOfficeFile {
     ...
     */
    private static final Set<String> ZIP_OFFICE_EXTENSIONS = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
     ...
     */
    public static final Map<String, Function<byte[], Boolean>> OFFICE_MAGIC_MAP;
    ...
    static {
        ...
        magicMap.put("doc", oleChecker);
        magicMap.put("xls", oleChecker);
        magicMap.put("ppt", oleChecker);

        // PDF
        magicMap.put("pdf", bytes -> bytes.length >= 4 && bytes[0] == 0x25 && bytes[1] == 0x50 && bytes[2] == 0x44 && bytes[3] == 0x46);

        // RTF
        magicMap.put("rtf", bytes -> bytes.length >= 5 && bytes[0] == 0x7B && bytes[1] == 0x5C && bytes[2] == 0x72 && bytes[3] == 0x74 && bytes[4] == 0x66);
        OFFICE_MAGIC_MAP = Collections.unmodifiableMap(magicMap);
    }

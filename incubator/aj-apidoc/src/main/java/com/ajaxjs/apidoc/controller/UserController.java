package com.ajaxjs.apidoc.controller;
...
public class UserController {
    ...
    @ApiStatus(ApiStatus.Status.STABLE)
    public User getUserById(@ApiParam(name = "id", description = "用户唯一标识符", example = "1001", required = true) @PathVariable Long id) {
        User user = new User();
        user.setId(id);
    ...
    @ApiStatus(ApiStatus.Status.STABLE)
    public User createUser(@ApiParam(name = "user", description = "用户信息，包含用户名、邮箱、年龄等基本信息", required = true) @RequestBody User user) {

package com.ajaxjs.apidoc;

import com.ajaxjs.apidoc.annotation.*;
import com.ajaxjs.apidoc.model.*;
...
public class ApiParser {
    ...
    public ApiDocumentation parseAll(String environment) {
        ...
        List<com.ajaxjs.apidoc.model.ApiGroup> groups = groupedApis.entrySet().stream()
                .map(entry -> {
                    com.ajaxjs.apidoc.model.ApiGroup group = new com.ajaxjs.apidoc.model.ApiGroup();
    ...
    private List<ApiInfo> parseController(Class<?> controllerClass) {
        ...
        com.ajaxjs.apidoc.annotation.ApiGroup apiGroup = controllerClass.getAnnotation(com.ajaxjs.apidoc.annotation.ApiGroup.class);
    ...
    private List<FieldInfo> parseReturnTypeFields(Class<?> returnType, java.lang.reflect.Type genericReturnType, Set<Class<?>> visited) {
        ...
        else if (Map.class.isAssignableFrom(returnType)) // 处理Map类型
            return parseMapTypeFields(genericReturnType, visited);
        else if (returnType.isArray()) { // 处理数组类型
        ...
        if (genericType instanceof java.lang.reflect.ParameterizedType) {
            ...
            if (actualTypes.length > 0) {
                ...
                if (elementType instanceof Class<?>) {
                    ...
                    return Collections.singletonList(containerField);
                }
            }
        }
        return new ArrayList<>();

package com.ajaxjs.webssh.controller;

import com.ajaxjs.webssh.service.SSHConnectionManager;
...
public class SSHWebSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        sessionLocks.put(session, new Object());  // 为每个会话创建同步锁
    }
    ...
    private void handleConnect(WebSocketSession session, JsonNode jsonNode) {
        try {
            ...
            sessionUsers.put(session, username);  // 存储用户信息
            String connectionId = connectionManager.createConnection(host, port, username, password);// 建立SSH连接
            sessionConnections.put(session, connectionId);
            ChannelShell channel = connectionManager.getChannel(connectionId); // 启动SSH通道
                ...
                try {
                    while (channel.isConnected() && session.isOpen()) {
                        if (in.available() > 0) {
                            ...
                            Thread.sleep(10);   // 没有数据时短暂休眠，避免CPU占用过高
                    }

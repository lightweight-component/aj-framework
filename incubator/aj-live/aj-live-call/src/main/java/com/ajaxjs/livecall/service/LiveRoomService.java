package com.ajaxjs.livecall.service;

import com.ajaxjs.livecall.model.LiveRoom;
import com.ajaxjs.livecall.model.User;
...
public class LiveRoomService {
    ...
    public boolean handleMicRequest(String roomId, String requestUsername, boolean accept, String broadcasterUsername) {
        LiveRoom room = liveRooms.get(roomId);
        if (room == null || !room.isActive() ||
        ...
        // 通知申请者结果
        try {
            log.info("Sending mic response to user {}: {}", requestUsername, accept ? "ACCEPTED" : "REJECTED");
            ...
            messagingTemplate.convertAndSendToUser(requestUsername, "/queue/mic-response", response);   // 发送消息给观众

            log.info("Mic response sent successfully");
        } catch (Exception e) {
            log.error("Failed to send mic response: {}", e.getMessage(), e); // 下面这行代码是不是会吞掉异常
            return false;
        }

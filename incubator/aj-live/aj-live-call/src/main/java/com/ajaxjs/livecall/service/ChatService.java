...
import com.ajaxjs.livecall.model.ChatMessage;
...
public class ChatService {
    ...
    public void sendMessage(ChatMessage message) {
        ...
        messagingTemplate.convertAndSend("/topic/room/" + message.getRoomId(), message);    // 将消息广播到直播间
        roomMessages.computeIfAbsent(message.getRoomId(), k -> new ArrayList<>()).add(message); // 存储消息

        List<ChatMessage> messages = roomMessages.get(message.getRoomId());// 限制消息数量
    ...
    public void sendSignalingMessage(String roomId, String sender, String receiver, String content) {
        ...
        // 发送给特定用户
        messagingTemplate.convertAndSendToUser(receiver, "/queue/signal", message);

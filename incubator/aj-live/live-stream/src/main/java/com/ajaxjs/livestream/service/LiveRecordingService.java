...
//    @EnableTransaction
    public LiveRecording startRecording(Long roomId) {
        ...
        Entity.instance().input(recording).create();
...
//    @EnableTransaction
    public LiveRecording stopRecording(Long recordingId) {
        LiveRecording recording = Sql.instance().input("SELECT * FROM live_recording WHERE id = ?", recordingId).query(LiveRecording.class);
        ...
        recording.setUpdatedAt(LocalDateTime.now());
        Entity.instance().input(recording).update();
        // 异步停止录制进程并上传文件
     ...
     * 获取直播回放列表
     */
    public PageResult<LiveRecording> getRecordings(Long roomId, int page, int size) {
        return Sql.instance().input("SELECT * FROM live_recording WHERE status = 3 AND room_id = ? ORDER BY start_time DESC", roomId).page(LiveRecording.class, page, size);
    }
    ...
    private void startRecordingProcess(LiveRoom liveRoom, LiveRecording recording) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                log.info("录制进程结束, roomId={}, recordingId={}, exitCode={}", liveRoom.getId(), recording.getId(), exitCode);
                ...
                Entity.instance().input(failedRecording).update();
            }
        });
    }
    ...
    private void uploadRecording(LiveRecording recording, File file) {
        try {
            ...
            Entity.instance().input(processingRecording).update();
            ...
            Entity.instance().input(updatedRecording).update();
            log.info("录制文件上传完成, recordingId={}, fileSize={}, duration={}s", recording.getId(), fileSize, duration);

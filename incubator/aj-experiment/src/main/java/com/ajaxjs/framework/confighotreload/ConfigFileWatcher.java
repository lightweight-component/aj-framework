...
//    @PostConstruct
    public void init() throws IOException {
        ...
        configDir.register(watchService, StandardWatchEventKinds.ENTRY_MODIFY); // 注册文件修改事件（ENTRY_MODIFY）
        ...
        executor.submit(() -> {
            while (true) {
                try {
                    ...
                    if (System.currentTimeMillis() - lastProcessTime < EVENT_DEBOUNCE_TIME) // 防抖检查：忽略短时间内重复事件
                        continue;

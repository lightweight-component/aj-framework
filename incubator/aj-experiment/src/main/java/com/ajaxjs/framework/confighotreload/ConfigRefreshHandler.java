...
public class ConfigRefreshHandler implements ApplicationContextAware {
    ...
    public void refresh() {
        try {
            Properties properties = loadConfigFile(); // 1. 重新读取配置文件内容
            Set<String> changeKeys = updateEnvironment(properties); // 2. 更新Environment中的属性
            if (!changeKeys.isEmpty())// 3. 重新绑定所有@ConfigurationProperties Bean
    ...
    private Properties loadConfigFile() throws IOException {
        Resource resource = new ClassPathResource("application.yaml");// 使用Spring工具类读取classpath下的配置文件
        YamlPropertiesFactoryBean yamlFactory = new YamlPropertiesFactoryBean();
        yamlFactory.setResources(resource);

        Properties properties = yamlFactory.getObject();// 获取解析后的Properties对象

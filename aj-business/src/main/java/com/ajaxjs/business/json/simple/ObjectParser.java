...
public class ObjectParser {
    ...
    public Object nextObject() {
        if (pos >= json.length()) throw new JsonException("unexpected string end.");
        ...
        switch (c) {
            ...
            case 'T'://可能是true
                pos++;
                if (checkChars('r', 'u', 'e')) return true;

                throw new JsonException("invalid boolean value");
            case 'f':
            case 'F': //可能是false
                pos++;
                if (checkChars('a', 'l', 's', 'e')) return false;

                throw new JsonException("invalid boolean value");
            case 'n':
            case 'N': //可能是null
                pos++;
                if (checkChars('u', 'l', 'l')) return null;
    ...
    private Object parseNumber(char c) {
        ...
        if (numberBuilder.length() > 0) {
            ...
            try {
                if (containsPoint) return Double.valueOf(numberString);
                else return Integer.valueOf(numberString);
    ...
    private JsonArray parseArray() {
        ...
        while (c != ']') {
            if (flag) {
                if (c != ',') throw new JsonException("illegal json array.");
    ...
    private JsonObject parseObject() {
        ...
        while (c != '}') {
            if (flag) {
                if (c != ',') throw new JsonException("illegal json object.");

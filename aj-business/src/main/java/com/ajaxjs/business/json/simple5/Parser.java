...
/**
 * 一起写一个JSON解析器
 * <a href="https://blog.csdn.net/weixin_30920513/article/details/98984680">...</a>
 * https://github.com/absfree/SimpleJSON/tree/master
 */
public class Parser {
    ...
    private JObject object() {
        ...
        } else if (isToken(TokenType.STRING)) map = key(map);
    ...
    private Map<String, Value> key(Map<String, Value> map) {
        ...
        if (!isToken(TokenType.COLON)) throw new JsonParseException("Invalid JSON input.");
        else {
            tokenizer.next(); //consume ':'

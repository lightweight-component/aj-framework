...
import com.ajaxjs.business.json.simple2.JsonParseException;
...
public class Tokenizer {
    ...
    private Token start() throws IOException, JsonParseException {
        ...
        while (true) {   //先读一个字符，若为空白符（ASCII码在[0, 20H]上）则接着读，直到刚读的字符非空白符
            if (!readerChar.hasMore()) return new Token(TokenType.END_DOCUMENT, null);
            ...
            if (ch == '\\') {   // 处理转义字符
                ...
                if (ch == 'u') {   // 处理 Unicode 编码，形如 \u4e2d。且只支持 \u0000 ~ \uFFFF 范围内的编码
                    for (int i = 0; i < 4; i++) {
                        ...
                        else throw new JsonParseException("Invalid character");
                    }
                }
    ...
    private String readFracAndExp() throws IOException {
        ...
        if (ch == '.') {
            ...
            if (!isDigit(ch)) throw new JsonParseException("Invalid frac");
            ...
            if (isExp(ch)) {    // 处理科学计数法
                ...
                if (ch != (char) -1) readerChar.back();
            }
        } else if (isExp(ch)) {
            sb.append(ch);
            sb.append(readExp());
        } else readerChar.back();
        ...
        if (ch == '+' || ch == '-') {
            ...
            if (isDigit(ch)) {
                ...
                if (ch != (char) -1)    // 读取结束，不用回退
                    readerChar.back();
            } else throw new JsonParseException("e or E");
        } else throw new JsonParseException("e or E");
    ...
    private Token readBoolean() throws IOException {
        if (readerChar.peek() == 't') {
            ...
            if (!(readerChar.next() == 'a' && readerChar.next() == 'l' && readerChar.next() == 's' && readerChar.next() == 'e'))
                throw new JsonParseException("Invalid json string");

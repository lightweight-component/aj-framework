...
public class DownloadUtil {
     ...
     * 文件下载
     *
     * @param fileUrl 文件 url，如:<a href="https://img3.doubanio.com//view//photo//s_ratio_poster//public//p2369390663.webp">...</a>
     * @param path    存放路径，如: /opt/img/xxx/my.webp
     */
    public static void download(String fileUrl, String path) {
        // 判断存储文件夹是否已经存在或者创建成功
        if (!createFolderIfNotExists(path)) {
            log.warn("We can't create folder:{}", getFolder(path));
            return;
        }
        ...
        try {
            ...
            try (InputStream in = conn.getInputStream(); FileOutputStream out = new FileOutputStream(path)) {
                ...
                while (-1 != (len = in.read(arr))) out.write(arr, 0, len);

                out.flush();
                conn.disconnect();
            }
        } catch (Exception e) {
            log.warn("Fail to download: {} by {}", fileUrl, e.getMessage());
        }
    }
    ...
    private static boolean createFolderIfNotExists(String path) {
        ...
        if (folderName.equals(path)) return true;
    ...
    public static void batch(Map<String, String> resourceMap) {
        if (resourceMap == null || resourceMap.isEmpty()) return;
    ...
    static class DownloadWorker implements Runnable {
        ...
        @Override
        public void run() {
            log.warn("Start batch:[{}] into: [{}]", url, path);
            DownloadUtil.download(url, path);
            log.debug("Download:[{}] into: [{}] is done", url, path);
            latch.countDown();
        }
